...
from plot_common import (
    ...
    get_circuit_name,
    mean,
    get_final_rows_streaming
)


CSV_FILE = max(
    glob.glob("../../out/build/x64-Debug/experiment_results/experiment_sa_sweeps_*.csv"),
...
def main():
    ...
    for sweep_parameter in sweep_parameters:
        ...
        plt.title(f"SA Parameter Sensitivity per Circuit: {sweep_parameter}")
        plt.yscale("log")
        plt.grid(True)
